...
#--------------------------------------
#    Exploracion
# dim es dimension, numero de observaciones y de columnas. 
#Colnames Nombre de las variables
# str tipo de variables 
#anyNA si hay datos faltantes Falso/verdadero
#summary medias de tencia central
...
color=c("aquamarine3","cornflowerblue")
...
# 1.- Creación de un vector de color
color=c("blueviolet", "darkblue", "green")

# 2.- Creación del gráfico
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "red", fill=color)+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="violet", fill="hotpink")+
  ggtitle("Histograma")+
...
GD2<-ggplot(penguins, aes(x=largo_pico_mm, y=grosor_pico_mm))+
  geom_point(aes(color=especie))+
  scale_color_manual(values=c("violet","green","blue"))+
  ggtitle("Gráfico de dispersión")+
  xlab("Largo del pico (mm)")+
  ylab("Grosor del pico (mm)")+
  theme_light()
